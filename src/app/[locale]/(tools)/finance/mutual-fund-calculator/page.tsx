...
import enTool from '@/data/tools/finance/mutual-fund-calculator-en.json';
import zhTool from '@/data/tools/finance/mutual-fund-calculator-zh.json';
...
export async function generateMetadata({
  ...
  const entry = catalogs[l]?.find((it) => it.id === 'mutual-fund-calculator') || catalogs.en.find((it) => it.id === 'mutual-fund-calculator');

  const toolName: string = entry?.name ?? 'InvestmentCalculator';
  const description: string = entry?.description ?? 'Professional financial calculator for accurate calculations and planning.';
  const baseKeywords = generateOptimizedKeywords('mutual-fund-calculator', 'finance', 'InvestmentCalculator');
  ...
  const canonicalPath = `${l === "en" ? "" : "/" + l}/finance/mutual-fund-calculator`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: '/finance/mutual-fund-calculator',
        zh: '/zh/finance/mutual-fund-calculator'
      }
    },
