...
import enTool from '@/data/tools/finance/rewards-calculator-en.json';
import zhTool from '@/data/tools/finance/rewards-calculator-zh.json';
...
export async function generateMetadata({
  ...
  const entry = catalogs[l]?.find((it) => it.id === 'rewards-calculator') || catalogs.en.find((it) => it.id === 'rewards-calculator');

  const toolName: string = entry?.name ?? 'RewardsCalculator';
  const description: string = entry?.description ?? 'Professional financial calculator for accurate calculations and planning.';
  const baseKeywords = generateOptimizedKeywords('rewards-calculator', 'finance', 'RewardsCalculator');
  ...
  const canonicalPath = `${l === "en" ? "" : "/" + l}/finance/rewards-calculator`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: '/finance/rewards-calculator',
        zh: '/zh/finance/rewards-calculator'
      }
    },
...
export default async function RewardsCalculatorPage({
  params
}: {
  params: Promise<{ locale: string }>
}) {
