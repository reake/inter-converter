...
import enTool from '@/data/tools/finance/capital-gains-tax-calculator-en.json';
import zhTool from '@/data/tools/finance/capital-gains-tax-calculator-zh.json';
...
export async function generateMetadata({
  ...
  const entry = catalogs[l]?.find((it) => it.id === 'capital-gains-tax-calculator') || catalogs.en.find((it) => it.id === 'capital-gains-tax-calculator');

  const toolName: string = entry?.name ?? 'TaxCalculator';
  const description: string = entry?.description ?? 'Professional financial calculator for accurate calculations and planning.';
  const baseKeywords = generateOptimizedKeywords('capital-gains-tax-calculator', 'finance', 'TaxCalculator');
  ...
  const canonicalPath = `${l === "en" ? "" : "/" + l}/finance/capital-gains-tax-calculator`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: '/finance/capital-gains-tax-calculator',
        zh: '/zh/finance/capital-gains-tax-calculator'
      }
    },
