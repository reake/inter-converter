...
import enTool from '@/data/tools/finance/trust-calculator-en.json';
import zhTool from '@/data/tools/finance/trust-calculator-zh.json';
...
export async function generateMetadata({
  ...
  const entry = catalogs[l]?.find((it) => it.id === 'trust-calculator') || catalogs.en.find((it) => it.id === 'trust-calculator');

  const toolName: string = entry?.name ?? 'TrustCalculatorCalculator';
  const description: string = entry?.description ?? 'Professional financial calculator for accurate calculations and planning.';
  const baseKeywords = generateOptimizedKeywords('trust-calculator', 'finance', 'TrustCalculatorCalculator');
  ...
  const canonicalPath = `${l === "en" ? "" : "/" + l}/finance/trust-calculator`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: '/images/og-trust-calculator.jpg',
          width: 1200,
          height: 630,
          alt: toolName
        }
    ...
    alternates: {
      ...
      languages: {
        en: '/finance/trust-calculator',
        zh: '/zh/finance/trust-calculator'
      }
    },
...
export default async function TrustCalculatorCalculatorPage({
  params
}: {
  params: Promise<{ locale: string }>
}) {
